...
export function FulfillmentWorkflowStepper({ status }: Props) {
  ...
  return (
    <div className="space-y-2">
      ...
      {terminal ? (
        <p className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-sm text-warning">
          Estado: {current}
        </p>
      ) : (
        <ol className="flex flex-wrap gap-1">
          {FULFILLMENT_STEPS.map((step, idx) => {
            ...
            return (
              <li
                key={step.status}
                className={`rounded-full px-2 py-0.5 text-caption font-medium uppercase tracking-wide ${
                  active
