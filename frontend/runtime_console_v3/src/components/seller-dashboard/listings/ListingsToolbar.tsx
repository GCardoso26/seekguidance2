...
export function ListingsToolbar({
  ...
  return (
    <div className="space-y-3 rounded-xl border border-white/10 bg-white/[0.03] p-3" data-testid="listings-toolbar">
      ...
      {selectedCount > 0 && (
        <div className="flex flex-wrap items-center gap-2 border-t border-white/10 pt-3">
          <span className="text-xs text-luxury-mist">{selectedCount} selecionado(s)</span>
          <button
            type="button"
            disabled={busy}
            onClick={onBulkActivate}
            className="rounded-lg bg-luxury-gold px-3 py-1.5 text-xs font-semibold text-luxury-onyx disabled:opacity-50"
          >
            Publicar
          </button>
          <button
