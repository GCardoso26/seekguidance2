  ...
  return (
    <div className="group relative">
      ...
      >
        ...
        {unread.count > 0 && (
          <span className="absolute right-1 top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-luxury-gold px-1 text-[10px] font-bold text-luxury-onyx">
      ...
      <div className="invisible absolute right-0 z-50 mt-2 w-80 rounded-xl border border-white/10 bg-luxury-obsidian opacity-0 shadow-xl transition group-hover:visible group-hover:opacity-100 group-focus-within:visible group-focus-within:opacity-100">
        <div className="border-b border-white/10 px-4 py-2 text-sm font-medium">Notificações</div>
        <ul className="max-h-80 overflow-y-auto">
          {items.length === 0 && <li className="px-4 py-6 text-center text-sm text-luxury-mist">Nenhuma notificação</li>}
          {items.map((n) => (
            <li key={n.id}>
              <button
                type="button"
              ...
              >
                ...
                {n.link && (
                  <span className="mt-1 inline-block text-xs text-luxury-gold">
