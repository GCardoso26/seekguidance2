    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex justify-end bg-black/50">
          <div className="flex h-full w-full max-w-sm flex-col bg-slate-900 text-slate-100 shadow-xl">
            ...
            <div className="flex flex-1 overflow-hidden">
              <div className="w-2/5 overflow-y-auto border-r border-slate-700">
                {friends.map((f: { id: string; display_name?: string; unread?: number }) => (
                  <button
                    key={f.id}
                    type="button"
                    onClick={() => setActiveFriend(f.id)}
                    className={`flex w-full items-center justify-between px-3 py-3 text-left text-sm hover:bg-slate-800 ${
                      activeFriend === f.id ? "bg-slate-800" : ""
                    }`}
                  >
                ...
                {activeFriend && (
                  ...
                  >
                    <input
                      ...
                      className="min-h-[44px] flex-1 rounded-lg border border-slate-600 bg-slate-800 px-3 text-sm"
