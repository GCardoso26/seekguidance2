  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-luxury-mist">Dashboard do lojista</p>
      </div>
      ...
      <section>
        <div className="mb-3 flex items-center justify-between">
          ...
          {onRefreshOrders && (
            <button type="button" onClick={onRefreshOrders} className="text-sm text-luxury-gold">
              Atualizar
            </button>
          )}
        </div>
        <div className="space-y-3">
          {orders.length === 0 && <p className="text-luxury-mist">Nenhum pedido ainda.</p>}
          {orders.map((order) => (
