...
export function normalizeCardListing(raw: ListingLike, cardId?: string): CardListing {
  ...
  return {
    ...
    sellerName,
    sellerReputation: Number.isFinite(raw.sellerReputation) ? Number(raw.sellerReputation) : 0,
    sellerReviewCount: Number.isFinite(raw.sellerReviewCount) ? Number(raw.sellerReviewCount) : 0,
    storeVerificationStatus: raw.storeVerificationStatus ?? null,
    storeAcceptsPix: Boolean(raw.storeAcceptsPix),
    storeAcceptsCard: Boolean(raw.storeAcceptsCard),
