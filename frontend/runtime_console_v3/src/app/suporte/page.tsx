...
export default function SuportePage() {
  return (
    <MobileLayout>
      <main className="container mx-auto max-w-lg px-4 py-10 text-luxury-frost">
        <Link href="/loja" className="text-sm text-luxury-mist hover:text-white">
          ← Voltar
        </Link>
        ...
        <section className="mt-8 space-y-4 rounded-xl border border-white/10 bg-white/5 p-5">
          <h2 className="font-semibold text-luxury-gold">Contato</h2>
          <p className="text-sm text-luxury-mist">
            Envie um e-mail com o assunto do problema e, se possível, o e-mail da sua conta Google.
          </p>
          <Button asChild className="w-full bg-luxury-gold text-luxury-onyx">
            <a href={`mailto:${SUPPORT_EMAIL}?subject=Suporte%20Judge%20TCG`}>{SUPPORT_EMAIL}</a>
          </Button>
        </section>

        <section className="mt-6 space-y-3 text-sm text-luxury-mist">
