...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ storeId: string }> }) {
  ...
  try {
    const res = await fetch(
      `${TOURNAMENT_API_BASE}/runtime/judge/marketplace/shop/stores/${encodeURIComponent(storeId)}/dashboard`,
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ storeId: string }> }) {
  ...
  try {
    ...
    const res = await fetch(
      `${TOURNAMENT_API_BASE}/runtime/judge/marketplace/shop/stores/${encodeURIComponent(storeId)}/products`,
      {
        method: "POST",
