    ...
    <MobileLayout>
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mt-2 flex items-center justify-between">
          ...
          <Link href="/leagues/create" className="rounded-lg bg-luxury-gold px-4 py-2 text-sm font-semibold text-luxury-onyx">
            Criar liga
          </Link>
        </div>
        ...
        <div className="mt-6 space-y-4">
          {(leagues as Array<Record<string, unknown>>).map((l) => (
            <Link
              key={String(l.id)}
              href={`/leagues/${l.id}`}
              className="block rounded-xl border border-white/10 p-4 hover:border-luxury-gold/50"
            >
