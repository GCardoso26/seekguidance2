"""replay_temporal_alignment_service — wiring operacional sobre repositórios existentes (sem DB pesado)."""
...
def replay_temporal_alignment_service_stub(replay_ref: str) -> dict[str, Any]:
    snap = replay_temporal_history_record_stub(replay_ref)
    lin = replay_lineage_append_stub(replay_ref, lineage=snap.get("lineage", {}))
    rec = replay_runtime_reconcile_stub(replay_ref)
    integ = replay_snapshot_put_stub(replay_ref)
    name = "replay_temporal_alignment_service"
