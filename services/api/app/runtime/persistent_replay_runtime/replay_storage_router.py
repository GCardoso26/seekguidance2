"""replay_storage_router — wiring operacional sobre repositórios existentes (sem DB pesado)."""
...
def replay_storage_router_stub(replay_ref: str) -> dict[str, Any]:
    snap = replay_snapshot_put_stub(replay_ref)
    lin = replay_governance_persist_stub(replay_ref, lineage=snap.get("lineage", {}))
    rec = replay_consistency_persist_stub(replay_ref)
    integ = replay_integrity_record_stub(replay_ref)
    name = "replay_storage_router"
