"""replay_reconciliation_service — wiring operacional sobre repositórios existentes (sem DB pesado)."""
...
def replay_reconciliation_service_stub(replay_ref: str) -> dict[str, Any]:
    snap = replay_runtime_reconcile_stub(replay_ref)
    lin = replay_diff_store_stub(replay_ref, lineage=snap.get("lineage", {}))
    rec = replay_consistency_persist_stub(replay_ref)
    integ = replay_governance_persist_stub(replay_ref)
    name = "replay_reconciliation_service"
