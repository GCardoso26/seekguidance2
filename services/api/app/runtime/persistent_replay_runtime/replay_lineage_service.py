"""replay_lineage_service — wiring operacional sobre repositórios existentes (sem DB pesado)."""
...
def replay_lineage_service_stub(replay_ref: str) -> dict[str, Any]:
    snap = replay_lineage_append_stub(replay_ref)
    lin = replay_temporal_history_record_stub(replay_ref, lineage=snap.get("lineage", {}))
    rec = replay_branch_archive_stub(replay_ref)
    integ = replay_runtime_index_upsert_stub(replay_ref)
    name = "replay_lineage_service"
