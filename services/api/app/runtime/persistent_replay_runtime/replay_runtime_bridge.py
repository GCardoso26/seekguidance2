"""replay_runtime_bridge — wiring operacional sobre repositórios existentes (sem DB pesado)."""
...
def replay_runtime_bridge_stub(replay_ref: str) -> dict[str, Any]:
    snap = replay_snapshot_put_stub(replay_ref)
    lin = replay_runtime_index_upsert_stub(replay_ref, lineage=snap.get("lineage", {}))
    rec = replay_runtime_reconcile_stub(replay_ref)
    integ = replay_lineage_append_stub(replay_ref)
    name = "replay_runtime_bridge"
