...
async def list_featured_sellers(session: AsyncSession, *, limit: int = 6) -> list[dict[str, Any]]:
    ...
    Um owner pode ter várias stores; DISTINCT ON (pp.id) evita cards duplicados
    (React key collision) e escolhe a store com melhor rating/listagens.
    """
        ...
        await session.execute(
            text(
                ...
                FROM (
                  ...
                  ORDER BY
                    pp.id,
                    COALESCE(s.average_rating, 0) DESC NULLS LAST,
                    COALESCE(lc.cnt, 0) DESC,
                    COALESCE(sc.sales, 0) DESC
                ) featured_one_per_seller
