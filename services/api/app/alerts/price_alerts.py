...
VALID_PRICE_CONDITIONS = frozenset({"below", "above"})
...
async def check_price_alerts(session: AsyncSession) -> dict[str, int]:
    ...
    for raw in rows:
        ...
        pcond = alert["price_condition"]
        fired = (pcond == "below" and current <= target) or (pcond == "above" and current >= target)
        if not fired:
            continue
        ...
        triggered_display = f"R$ {current / 100:.2f}"
        card_url = f"{app_base}/cards/{alert['card_id']}"

        user_id = str(alert["user_id"])
        if not alert.get("email_notified"):
        ...
        if not alert.get("push_notified"):
            await notification_service.send(
                session,
                "price_alert:triggered",
                player_ids=[user_id],
                body=f"{card_name} atingiu {triggered_display}",
