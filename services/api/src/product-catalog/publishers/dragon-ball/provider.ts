...
async function fetchSets(): Promise<PublisherSetSeed[]> {
  const url = process.env.DBFW_SETS_URL?.trim() || "https://cdn.judgetcg.example/publishers/dbfw/sets.json";
  try {
  ...
  return [
    {
      ...
      imageUrl: "https://cdn.judgetcg.example/publishers/dragon-ball/s1.webp",
    },
  ];
}
