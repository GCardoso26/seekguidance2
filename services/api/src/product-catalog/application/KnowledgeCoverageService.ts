...
export interface KnowledgeCoverageReport {
  ...
  totals: {
    ...
    withLifecycleFromMetadata?: number;
...
export class KnowledgeCoverageService {
  ...
  /** Read-only report — never writes (BUG-V4-004). */
  async computeReport(): Promise<KnowledgeCoverageReport> {
    const totals = await this.db.query<{
      ...
      with_lifecycle_non_default: string;
      with_lifecycle_metadata: string;
    ...
    const withLifecycleNonDefault = Number(t.with_lifecycle_non_default);
    const withLifecycleMetadata = Number(t.with_lifecycle_metadata);
    ...
    // AVAILABLE is a valid official/default state — do not penalize (BUG-V4-007)
    const lifecycleCoverage = pct(withLifecycle, products);
    ...
    return {
      ...
      totals: {
        ...
        withSpecs,
        withMetadata,
        withLifecycleNonDefault: withLifecycleNonDefault,
        withLifecycleFromMetadata: withLifecycleMetadata,
        withCollection,
  ...
  /** Scheduler / admin POST — persist snapshot after computing. */
  async refreshAndPersist(): Promise<KnowledgeCoverageReport> {
