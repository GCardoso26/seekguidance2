...
- [ ] Criar `TcgCsvSealedProvider.ts` (seção 5.1) com o mapa de `categoryId`.
- [ ] Filtrar somente selados (descartar cartas) e mapear subcategoria/`product_type`.
- [ ] Emitir `images[].sourceUrl` no padrão `_in_1000x1000.jpg`.
- [ ] Registrar em `providers/registry.ts` (`catalog.sync.sealed`).
- [ ] (Opcional) linha em `product_catalog.provider_registry`.
- [ ] Confirmar `tcgplayer-cdn.tcgplayer.com` no `next.config.mjs` (já presente).
