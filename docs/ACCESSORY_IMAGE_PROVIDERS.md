...
- [ ] Para **Shopify** (Dragon Shield, Ultra Pro, Heavy Play): adapter `products.json` por categoria.
...
- [ ] Mapear `product_type`/tags → `ProductCategory`/subcategoria.
- [ ] Adicionar hosts no `next.config.mjs` **ou** ingerir para R2 (`PRODUCT_CATALOG_R2_PUBLIC_BASE`).
- [ ] Registrar providers em `providers/registry.ts` nos job keys corretos.
