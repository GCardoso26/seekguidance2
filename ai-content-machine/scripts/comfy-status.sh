...
echo "ERROR → Comfy crash-loop ou ainda a arrancar."
echo "  docker compose --profile comfy logs --tail 80 comfy"
echo "  ModuleNotFoundError: requests → git pull && docker compose --profile comfy up -d --build comfy"
echo "  logs só com 'Requirement already satisfied: requests' → CMD envenenado por docker commit;"
echo "    force entrypoint no compose ou restaura CMD (ver docs/COMFYUI_A1_VS_PC.md)."
