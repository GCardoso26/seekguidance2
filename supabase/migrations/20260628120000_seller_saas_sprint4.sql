-- Sprint 4: planos lojista, BuyList, CRM, PDV
SET search_path TO tcg_judge, public;

-- Expandir planos de assinatura
...
-- BuyList: ofertas de compra de coleção
...
CREATE POLICY "Buylists public read active" ON buylists
  FOR SELECT USING (status = 'active');

DROP POLICY IF EXISTS "Store owners manage buylists" ON buylists;
CREATE POLICY "Store owners manage buylists" ON buylists
  FOR ALL USING (
...
CREATE POLICY "Buylist items public read" ON buylist_items
  FOR SELECT USING (
...
CREATE POLICY "Store owners manage buylist items" ON buylist_items
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM buylists b
...
CREATE POLICY "Store owners manage submissions" ON buylist_submissions
  FOR ALL USING (
    EXISTS (SELECT 1 FROM stores s WHERE s.id = store_id AND s.owner_id = auth.uid()::text)
  );
...
CREATE POLICY "Store owners manage customers" ON store_customers
  FOR ALL USING (
    EXISTS (SELECT 1 FROM stores s WHERE s.id = store_id AND s.owner_id = auth.uid()::text)
  );

DROP POLICY IF EXISTS "Store owners manage pdv" ON pdv_sales;
CREATE POLICY "Store owners manage pdv" ON pdv_sales
  FOR ALL USING (
    EXISTS (SELECT 1 FROM stores s WHERE s.id = store_id AND s.owner_id = auth.uid()::text)
  );
