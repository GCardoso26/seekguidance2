-- Judge TCG v2: escrow protegido + cache de health score do mercado
...
CREATE TABLE IF NOT EXISTS escrow_transactions (
  ...
  payment_method TEXT NOT NULL DEFAULT 'pix' CHECK (payment_method IN ('pix', 'credit_card', 'stripe')),
...
CREATE TRIGGER escrow_status_history
  ...
  FOR EACH ROW
  EXECUTE FUNCTION tcg_judge.log_escrow_status_change();
...
-- Health score semanal por TCG (cache)
...
-- Estender alertas: variação percentual
...
COMMENT ON TABLE escrow_transactions IS 'Compra protegida — fundos retidos até confirmação';
COMMENT ON TABLE market_health_scores IS 'Índice de saúde do mercado por TCG (atualizado via cron)';
